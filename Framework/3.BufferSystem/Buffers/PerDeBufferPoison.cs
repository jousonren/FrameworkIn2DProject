...
public class PerDeBufferPoison : BufferStateBase
{
	...
	{
		CurrCtrl.GetComponentInChildren<BeiJi> ().OnSubHp (CurrArgs.m_iValue1, new Color (0.41f, 0f, 0.53f, 1f));
	...
	public override bool OnUpdate ()
	{
		...
		if (!CurrArgs.isOver) {
			if ((lastTime - CurrArgs.m_ContinuousTime) >= CurrArgs.m_fValue1) {
				lastTime = CurrArgs.m_ContinuousTime;
				CurrCtrl.GetComponentInChildren<BeiJi> ().OnSubHp (CurrArgs.m_iValue1, new Color (0.04f, 0.62f, 0.08f, 1f));
			}
		}
		return true;
	}
